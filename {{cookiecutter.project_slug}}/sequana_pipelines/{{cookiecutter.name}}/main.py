...
def main(args=None):
    options = Options(prog="{{cookiecutter.name}}")
    if args is None:
        args = sys.argv
    ...
    runme = "#!/bin/bash\nsnakemake -s {{cookiecutter.name}}.rules"
    ...
    with open("{}/{{cookiecutter.name}}.sh".format(outdir), "w") as fout:
        fout.write(runme)
    ...
    msg = "Once ready, execute the script {{cookiecutter.name}}.sh using \n\n\t"
    if options.run_mode == "slurm":
        msg += "cd {}; sbatch {{cookiecutter.name}}.sh\n\n"
    else:
        msg += "cd {}; sh {{cookiecutter.name}}.sh\n\n"
